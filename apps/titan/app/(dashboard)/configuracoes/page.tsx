...
import { ArrowLeft, Loader2, User, Mail, Lock, Save, Eye, EyeOff, Award, Building2 } from 'lucide-react'
...
export default function ConfiguracoesPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const [{ data: kdData }, { data: fedData }] = await Promise.all([
        ...
        if (fedData) {
          setProfile(fedData as ProfileData)
          setForm(fedData as ProfileData)
        }
      } catch (err) {
        console.error(err)
      } finally {
  ...
  const saveEmail = async () => {
    ...
    try {
      ...
      setEmailMsg({ type: 'success', text: 'Confirmação enviada para o novo email. Verifique sua caixa de entrada.' })
    } catch (err: any) {
  ...
  const kyuDanAtual = kyuDans.find(k => k.id === Number(profile?.kyu_dan_id))
      ...
      <div className="max-w-3xl mx-auto px-4 py-10 space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <p className="text-white text-sm font-medium">
                  {kyuDanAtual ? `${kyuDanAtual.cor_faixa} | ${kyuDanAtual.kyu_dan}` : '—'}
                </p>
              </div>
              <div>
                <p className="text-gray-400 text-sm">Academia</p>
                <p className="text-white text-sm font-medium">{profile.academias || '—'}</p>
              </div>
              <div>
                <p className="text-gray-400 text-sm">Status do Plano</p>
                <p className="text-white text-sm font-medium">{profile.status_plano || '—'}</p>
              </div>
              <div>
                <p className="text-gray-400 text-sm">Status do Membro</p>
                <p className="text-white text-sm font-medium">{profile.status_membro || '—'}</p>
              </div>
              <div>
                <p className="text-gray-400 text-sm">Data de Nascimento</p>
                <p className="text-white text-sm font-medium">{profile.data_nascimento || '—'}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {profile ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <Field label="Nome Completo">
                  <input className={inputCls} value={form.nome_completo || ''} onChange={e => set('nome_completo', e.target.value)} />
                </Field>
              </div>
              <Field label="Nome no Patch">
                <input className={inputCls} value={form.nome_patch || ''} onChange={e => set('nome_patch', e.target.value)} />
              </Field>
              <Field label="Gênero">
                <select className={selectCls} value={form.genero || ''} onChange={e => set('genero', e.target.value)}>
              ...
              <Field label="Email (na federação)">
              ...
              <Field label="Nacionalidade">
                <select className={selectCls} value={form.nacionalidade || ''} onChange={e => set('nacionalidade', e.target.value)}>
                  <option value="">Selecione</option>
              ...
              <Field label="País">
                <select className={selectCls} value={form.pais || ''} onChange={e => set('pais', e.target.value)}>
                  <option value="">Selecione</option>
                  {PAISES_TOP.map(p => <option key={`top-${p}`} value={p}>{p}</option>)}
                  <option disabled>──────────</option>
                  {PAISES.filter(p => !PAISES_TOP.includes(p)).map(p => <option key={p} value={p}>{p}</option>)}
              ...
              <Field label="Tamanho do Patch">
                <select className={selectCls} value={form.tamanho_patch || ''} onChange={e => set('tamanho_patch', e.target.value)}>
                  <option value="">Selecione</option>
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-6 text-center text-gray-400">
            ...
            Nenhum perfil de filiação encontrado para este usuário.
          </div>
        )}
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-6 space-y-4">
          ...
          <p className="text-gray-400 text-sm">Email atual: <span className="text-white">{currentEmail}</span></p>
