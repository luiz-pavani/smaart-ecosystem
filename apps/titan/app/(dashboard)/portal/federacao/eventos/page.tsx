...
import { ArrowLeft, Calendar, MapPin, Trophy, Users, Loader2, ExternalLink, Plus } from 'lucide-react'
...
export default function EventosFederacaoPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const enriched: Evento[] = eventos.map((e: any) => ({
        ...e,
        total_inscritos: regCount[e.id] || 0,
      }))
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-400 mt-1">Calendário da federação</p>
            </div>
            <button
          ...
          {(['upcoming', 'past'] as const).map(t => (
            ...
            >
              {t === 'upcoming'
                ...
                : `Anteriores${past.length ? ` (${past.length})` : ''}`}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-12 text-center">
            ...
            <p className="text-gray-400 mb-2">
              {tab === 'upcoming' ? 'Nenhum evento próximo cadastrado' : 'Nenhum evento anterior'}
            </p>
            {tab === 'upcoming' && (
              <p className="text-gray-500 text-sm">
                Eventos criados via Smoothcomp aparecem automaticamente quando importados
              </p>
            )}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {list.map(evento => {
              ...
              return (
                <div
                  key={evento.id}
                  className="bg-white/5 backdrop-blur border border-white/10 rounded-xl p-6 hover:border-yellow-500/30 transition-all"
                >
                  <div className="flex items-start justify-between gap-3 mb-4">
                    <div className="flex items-start gap-3">
                      ...
                      <h3 className="text-lg font-semibold text-white leading-tight">{evento.nome}</h3>
                    </div>
                    {evento.status && (
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center gap-2 text-gray-400">
                      ...
                      <span>
                        <span className="text-white font-medium">{evento.total_inscritos}</span>
                        {' '}inscrito{evento.total_inscritos !== 1 ? 's' : ''}
                      </span>
                    </div>
                  </div>
