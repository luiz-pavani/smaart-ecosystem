-- Migration 016: Fix ALL RLS policies for master_access (FIXED VERSION)
...
CREATE POLICY "Academias - users view own"
  ON academias FOR SELECT
  USING (
    EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND academia_id = academias.id)
  );

CREATE POLICY "Academias - admins view their scope"
  ON academias FOR SELECT
  USING (
