-- Tabela de referência central de graduações (kyu/dan)
-- Fonte dos valores: planilha kyudan.csv
-- Objetivo: ser a referência oficial para graduações no sistema
...
BEGIN
  ...
  IF resolved_dan IS NOT NULL AND resolved_dan BETWEEN 1 AND 10 THEN
    ...
    WHERE kd.kyu_dan = resolved_dan::TEXT || 'º dan'
    LIMIT 1;
  ...
  IF normalized_graduacao LIKE '%DAN%' THEN
    ...
    IF dan_from_text IS NOT NULL AND dan_from_text BETWEEN 1 AND 10 THEN
      SELECT id INTO resolved_id FROM public.kyu_dan WHERE kyu_dan = dan_from_text::TEXT || 'º dan' LIMIT 1;
    ELSE
  ...
  ELSIF normalized_graduacao LIKE '%MŪKYŪ%' OR normalized_graduacao LIKE '%MUKYU%' OR normalized_graduacao LIKE '%MUKYU%' OR normalized_graduacao LIKE '%BRANCA%' THEN
...
COMMENT ON TABLE public.kyu_dan IS 'Tabela mestre de graduações kyu/dan. Referência oficial para graduações no sistema.';

DO $$
BEGIN
  IF to_regclass('public.user_fed_lrsj') IS NOT NULL THEN
    COMMENT ON COLUMN public.user_fed_lrsj.kyu_dan_id IS 'FK para graduação oficial em public.kyu_dan';
  END IF;

  IF to_regclass('public.atletas') IS NOT NULL THEN
    COMMENT ON COLUMN public.atletas.kyu_dan_id IS 'FK para graduação oficial em public.kyu_dan';
  END IF;
END;
$$;
